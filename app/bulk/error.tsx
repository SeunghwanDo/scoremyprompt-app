...
export default function BulkError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-dark via-surface to-dark flex items-center justify-center p-4">
      <div className="bg-surface border border-border rounded-lg p-8 max-w-md w-full text-center">
        ...
        <h2 className="text-xl font-bold text-white mb-2">Bulk Analysis Error</h2>
        <p className="text-gray-400 mb-6 text-sm">
          {process.env.NODE_ENV === 'production'
            ? 'Failed to process bulk analysis. Please try again.'
